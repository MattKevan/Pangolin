...
class SpeechTranscriptionService: ObservableObject {
    ...
    // MARK: - Language Detection and Translation Support

    /// Detects the primary language of a transcript
    func detectTranscriptLanguage(from transcript: String) -> String? {
        guard !transcript.isEmpty else { return nil }

        languageDetector.reset()
        languageDetector.processString(transcript)

        guard let dominantLanguage = languageDetector.dominantLanguage else {
            return nil
        }
    ...
    /// Determines if translation should be performed based on library settings and detected language
    func shouldTranslateTranscript(for video: Video, library: Library, detectedLanguage: String?) -> Bool {
        guard library.autoTranslateOnImport else { return false }

        guard let detectedLanguage = detectedLanguage else {
            print("🌐 TRANSLATION: No language detected, skipping translation")
            return false
        }

        let targetLanguage = library.translationTargetLanguage

        // Only translate if languages differ
        let shouldTranslate = detectedLanguage != targetLanguage
        print("🌐 TRANSLATION: Detected: \(detectedLanguage), Target: \(targetLanguage), Should translate: \(shouldTranslate)")

        return shouldTranslate
    }

    /// Creates a callback wrapper for progress tracking during background processing
    func transcribeVideo(_ video: Video, progressCallback: @escaping (Double, String) -> Void) async throws {
        // Update existing transcribeVideo method to accept progress callback
        await MainActor.run {
            isTranscribing = true
            progress = 0.0
            errorMessage = nil
        ...
        // Use progress callback instead of @Published properties for background tasks
        progressCallback(0.0, "Starting transcription...")

        // Perform transcription with progress updates via callback
        try await performTranscriptionWithCallback(video, progressCallback: progressCallback)
    ...
    private func performTranscriptionWithCallback(_ video: Video, progressCallback: @escaping (Double, String) -> Void) async throws {
        // This would be the actual transcription logic
        // For now, we'll simulate the process
        progressCallback(0.1, "Preparing audio...")

        // Simulate transcription work
        for i in 1...10 {
            try await Task.sleep(nanoseconds: 100_000_000) // 0.1 seconds
            let progress = Double(i) / 10.0
            progressCallback(progress, "Transcribing audio (\(Int(progress * 100))%)")
        }

        // Simulate language detection
        progressCallback(0.95, "Detecting language...")

        // Mock transcript for testing
        let mockTranscript = "This is a test transcript that would be generated by the speech recognition service."

        await MainActor.run {
            video.transcriptText = mockTranscript
            video.transcriptLanguage = "en" // Mock detected language
            video.transcriptDateGenerated = Date()
        }
